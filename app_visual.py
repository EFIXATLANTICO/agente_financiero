...
def _normalizar_texto_conciliacion(texto):
    ...
    txt = txt.replace("A", "a").replace("A", "e").replace("A", "i").replace("A3", "o").replace("Ao", "u")
...
def normalizar_texto_fianza(texto):
    ...
    texto = texto.replace("A", "a").replace("A", "e").replace("A", "i").replace("A3", "o").replace("Ao", "u")
    return texto
...
def extraer_importes_desde_texto_fianza(texto):
    ...
    patrones_contexto = [
        ...
        r'(\d+(:[.,]\d{1,2}))\s*a\s*(:de\s*)fianza',
        r'(\d+(:[.,]\d{1,2}))\s*a\s*(:de\s*)deposito',
        r'(\d+(:[.,]\d{1,2}))\s*a\s*(:de\s*)garantia',
    ]
    ...
    with col_e5:
        ...
        if estado_revision_actual == "descartada":
            ...
            if st.button("No es una fianza", key=f"descartar_fianza_{asiento_edicion_id}"):
                ...
                st.success("Marcada como no fianza. Ya no volverA a salir en pendientes.")
                st.rerun()
...
def pantalla_cuenta_resultados():
    st.markdown('<div class="section-title">Cuenta de pArdidas y ganancias</div>', unsafe_allow_html=True)
...
def pantalla_control_contable():
    ...
    if df_control.empty:
        ...
        if df_mostrar.empty:
            ...
            if "revisada" in df_mostrar.columns:
                df_mostrar["revisada"] = df_mostrar["revisada"].apply(lambda x: "sA" if x else "no")
            ...
            if incidencias_con_asiento.empty:
                st.info("Las incidencias filtradas no estAn asociadas a un asiento concreto.")
            else:
                opciones_incidencia = {}
                        ...
                        if lineas:
                            ...
                            with ac2:
                                if st.button("Aceptar incidencia", key=f"aceptar_incidencia_{asiento_id_sel}"):
                                    ...
                                    if resultado_revision.get("ok"):
                                        st.success("Incidencia marcada como revisada. Ya no aparecerA en el listado.")
                                        st.rerun()
                                    else:
                            ...
                            with ac4:
                                if st.button("Quitar revisada", key=f"quitar_revision_inc_{asiento_id_sel}"):
                                    ...
                                    with bc1:
                                        if st.button("SA, borrar asiento", key=f"confirmar_borrar_desde_control_si_{asiento_id_sel}"):
    ...
    if not st.session_state["confirmar_borrado_asiento"]:
        ...
        with col_b1:
            if st.button("SA, borrar asiento", key="confirmar_borrar_asiento_si"):
    ...
    if not st.session_state["confirmar_reset_contable"]:
        ...
        st.warning("ASeguro que quieres borrar toda la contabilidad")

        col1, col2 = st.columns(2)

        with col1:
            if st.button("SA, resetear todo", key="confirmar_reset_contable_si"):
...
def pantalla_facturas(cursor):
    ...
    with col_a:
        ...
        if estado_actual_tmp in ("pendiente", "vencida"):
            ...
            st.success("Esta factura ya estA cobrada/pagada.")
        ...
        if df.empty:
            ...
            with c2:
                ...
                if not st.session_state["confirmar_borrar_cliente"]:
                    ...
                    with cc1:
                        if st.button("SA, borrar", key="confirmar_borrar_cliente_si"):
        ...
        if df.empty:
            ...
            with c2:
                ...
                if not st.session_state["confirmar_borrar_proveedor"]:
                    ...
                    with cc1:
                        if st.button("SA, borrar", key="confirmar_borrar_proveedor_si"):
    ...
    if archivo is not None:
        ...
        if tipo_excel == "movimientos":
            ...
            if st.session_state.confirmar_importacion_movimientos:
                ...
                with col1:
                    if st.button("SA, importar movimientos"):
            ...
            if st.session_state.confirmar_importacion_facturas:
                ...
                with c1:
                    if st.button("SA, importar facturas"):
                        archivo_bytes = st.session_state["archivo_excel"].getvalue()
            ...
            if isinstance(resultado_facturas, dict) and resultado_facturas.get("ok", False):
                ...
                if resultado_facturas.get("num_errores", 0) > 0:
                    ...
                    for i, row in df_errores.iterrows():
                        ...
                        with col5:
                            if st.button("Y", key=f"editar_error_factura_{i}"):
            ...
            if "error_en_edicion_factura" in st.session_state:
                ...
                with st.container():
                    ...
                    if accion == "Corregir total":
                        ...
                        if st.button("a... Reintentar con total corregido", key="btn_reintentar_total_factura"):
                        ...
                        st.info("Se importarA como factura rectificativa o abono")
                        ...
                        if st.button("ai  Importar como abono", key="btn_importar_abono_factura"):
            ...
            if st.session_state["confirmar_importacion_pagos_proveedor"]:
                ...
                with c1:
                    if st.button("SA, importar pagos a proveedores", key="btn_confirmar_importar_pagos_proveedor"):
                        archivo_bytes = st.session_state["archivo_excel"].getvalue()
    ...
    st.markdown(
        ...
        ">
            ...
            <div style="
                ...
                width:100%;
            ">
                Y {usuario.get('username', '')} &nbsp;&nbsp;|&nbsp;&nbsp; Y {empresa.get('nombre', '')}
            </div>
...
def pantalla_incidencias_importacion():
    ...
    with col_acc_1:
        if st.button("Y1 Vaciar incidencias", key="vaciar_incidencias_importacion"):
            conn = get_connection()
            cursor = conn.cursor()
            try:
    ...
    # =========================
    # BLOQUE 2 - QUA HA PASADO
    # =========================
    st.markdown("## 2) QuA ha pasado")
    ...
    with st.expander("Ver datos tAcnicos originales"):
        st.json(datos)
